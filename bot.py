...
# --- Config persistencia y tiempos ---
VIP_DURATION_SECONDS = 30 * 24 * 3600            # 30 días
DB_PATH = "/mnt/data/puremuse.sqlite3"           # ruta de BD en Render (persistente)
...
def mp_create_preference_for_user(chat_id: int, title="PureMuse VIP – 30 days", qty=1, unit_price=99.0, currency_id="MXN") -> str:
    ...
    body = {
        ...
        "external_reference": str(chat_id)  # clave: pago → usuario
    }
...
def home():
    return "PureMuse Bot is up ✨ Stage 3 (VIP+DB)", 200
...
# Mercado Pago Webhook (activa VIP)
